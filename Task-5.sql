...
--  INNER JOINS - 	Matches in both tables
SELECT O.ORDER_ID, O.ORDER_TYPE, C.CUSTOMER_NAME
FROM ORDERS O
INNER JOIN CUSTOMERS C
ON O.CUSTOMER_ID = C.CUSTOMER_ID;

-- LEFT JOIN - All left table + matches from right
SELECT C.CUSTOMER_NAME, O.ORDER_TYPE
FROM CUSTOMERS C
LEFT JOIN ORDERS O
ON C.CUSTOMER_ID = O.CUSTOMER_ID;

-- RIGHT JOIN - All right table + matches from left
SELECT C.CUSTOMER_NAME, O.ORDER_TYPE
FROM CUSTOMERS C
RIGHT JOIN ORDERS O
ON C.CUSTOMER_ID = O.CUSTOMER_ID;
...
-- FULL JOIN - 	All from both tables Using UNION
SELECT C.CUSTOMER_NAME, O.ORDER_TYPE
FROM CUSTOMERS C
LEFT JOIN ORDERS O
ON C.CUSTOMER_ID = O.CUSTOMER_ID
...
-- Task - 6
-- CUSTOMER WHO ORDERED EXPENSIVE PRODUCT
...
SELECT ORDER_ID, ORDER_TYPE, CUSTOMER_ID
FROM ORDERS
WHERE CUSTOMER_ID IN (
    SELECT CUSTOMER_ID
    FROM CUSTOMERS
...
-- CUSTOMERS WITH WHO ORDERED MORE THAN 2 PROUCTS
SELECT CUSTOMER_NAME
FROM CUSTOMERS
WHERE CUSTOMER_ID IN (
    SELECT CUSTOMER_ID
...
-- CUSTOMERS WITH HIGHEST NO.OF ORDERS
